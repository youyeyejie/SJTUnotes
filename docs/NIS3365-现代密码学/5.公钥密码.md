...
- 定理：设 $n=pq$, $p$、$q$ 为 blum 整数，$x^2 \equiv a \bmod n$ 有解，则求解该同余式等价于分解大整数 $n$
	...
	- 证明：
		...
		- 若能求出四个解 $x_1, -x_1, x_2, -x_2$，且 $x_1$ 和 $x_2$ 模 $n$ 不同余且为小于 $n/2$ 的正整数，则由 $x_1^2 \equiv a \bmod p$ 和 $x_2^2 \equiv a \bmod q$ 可得 $x_1 \equiv x_2 \bmod n$，而 $x_1 + x_2 < n$, 不可能为 $n$ 的倍数，所以 $x_1 - x_2$ 和 $x_1 + x_2$ 分别含有 $n$ 的一个因子，也即
			...
			$$
			\begin{aligned} \gcd(x_1 - x_2, n)\\ \gcd(x_1 + x_2, n) \end{aligned}
			$$
...
- ECMQV 密钥交换算法
	...
	4. Alice 验证 $R_b$，计算其 $x$ 坐标 $\overline{R_b}$，计算共享密钥：
		...
		$$
		\begin{aligned} &s_a = r_a + \overline{R_b} k_a \bmod p \\ &K_{a,b} = s_a (Q_b + \overline{R_b} R_b) \end{aligned}
		$$

	5. Bob 验证 $R_a$，计算其 $x$ 坐标 $\overline{R_a}$，计算共享密钥：

		$$
		\begin{aligned} &s_b = r_b + \overline{R_a} k_b \bmod p \\ &K_{b,a} = s_b (Q_a + \overline{R_a} R_a) \end{aligned}
		$$
