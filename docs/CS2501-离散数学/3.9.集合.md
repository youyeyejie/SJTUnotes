...
- 若两个集合 $A$ 和 $B$ 没有公共元素，就称 $A$ 和 $B$ 是不相交的。这个定义也可以写成
	...
	$$
	A \text{ 和 } B \text{ 不相交 } \Leftrightarrow \neg(\exists x)(x \in A \wedge x \in B)
	$$
...
- 显然， $(\forall x)(x \notin \varnothing)$ 为真。
	...
	- $A = \varnothing$ 当且仅当 $A = \{x \mid x \neq x\}$.

	- $A \neq \varnothing$ 当且仅当 $A = \{x \mid (\exists y)(x=y)\}$.

- **定理**：

	- 对于任意的集合 $A$，$\varnothing \subseteq A$
...
- 此外，规定 $\cup \varnothing=\varnothing$，规定 $\cap \varnothing$ 无意义
...
- 同一律
	...
	$$
	\begin{aligned} A \cup \varnothing=A\\ A \cap E=A \end{aligned}
	$$

- 零律

	$$
	\begin{aligned} A \cup E=E\\ A \cap \varnothing=\varnothing\\ -\varnothing=E\\ -E=\varnothing \end{aligned}
	$$

- 补余律

	$$
	\begin{aligned} A \cup-A=E\\ A \cap-A=\varnothing \end{aligned}
	$$
...
- **经典传递集合**：
    ...
    - $\cdots$（即自然数的集合 $\mathbb{N}$，不断往上一个集合中加入它自身）
