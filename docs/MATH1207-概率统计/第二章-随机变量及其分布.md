...
#### 负二项分布 (Pascal 分布)
进行一个试验直到成功 $r$ 次，试验进行了 $k$ 次的分布律，称为 **负二项分布**，记为 $X\sim NB(r,p)$

$$
P(X=k) = \mathrm C_{k-1}^{r-1} p^r (1-p)^{k-r}, k = r, r+1, \cdots
$$
...
2. 对上式变换，若函数 $g(x)$ 在 $(-\infty, +\infty)$ 内严格单调，则 $F_Y(y) =P(X \le g^{-1}(y)) = F_X(g^{-1}(y))$
